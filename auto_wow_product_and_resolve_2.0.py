...
class ScreenCaptureOCR:
    ...
    def press_product_and_resolve(self):
        ...
        while True:
            print("正在检测价格列表区域...")
            # 截取背包剩余值区域并进行 OCR 识别
            bag_values = self.capture_screen_and_ocr(bag_value_region)

            if bag_values and len(bag_values) == 1:
                bag_value = bag_values[0]
                print(f"背包剩余值为：{bag_value}")

                if bag_value == bag_previous_value:
                    bag_value_same_value_count = bag_value_same_value_count + 1
                    print(f"当前连续识别到背包剩余值相同{bag_value_same_value_count}次。")
                    # 如果连续识别到背包剩余值相同100次 可能卡分解了 重启页面
                    if bag_value_same_value_count >= 100:
                        print("连续识别到背包剩余值大于100次，正在重启页面...")
                        self.refresh_and_reopen_produce_page()

                        bag_value_same_value_count = 0
                        continue
                else:
                    bag_previous_value = bag_value
                    bag_value_same_value_count = 0

                if bag_value <= self.preset_bag_value:
                    # 点击分解
                ...
                if flag:
                    ...
                    self.mouse_controller.perform_mouse_click(produce_position)
                    sleep(interval * 5)
                    print("正在制造...(flag = True)")
                    # 点击分解
                    self.mouse_controller.perform_mouse_click(resolve_position)
                    sleep(interval)
                    print("正在分解...(flag = True)")
                    ...
                    self.mouse_controller.perform_mouse_click(resolve_position)
                    sleep(interval * 5)
                    print("正在分解...(flag = False)")
                    # 点击制造
                    self.mouse_controller.perform_mouse_click(produce_position)
                    sleep(interval)
                    print("正在制造...(flag = False)")
